...
// Preserva o que o usuário está digitando entre re-renders do snapshot
const comentarioRascunho = {}
const motivoRascunho     = {}
...
function fixarComentario(id) {
  ...
  if (!motivo.trim()) {
    mostrarToast("Digite o motivo antes de fixar", "error")
    return
  }
  ...
  db.collection("solicitacoes").doc(id).update({
    ...
    // Trava o campo motivo — não pode mais editar
    if (motivoEl) {
      motivoEl.readOnly = true
      motivoEl.style.background = "#f1f5f9"
      motivoEl.style.color = "#475569"
      motivoEl.style.borderColor = "#cbd5e1"
      motivoEl.style.cursor = "not-allowed"
      motivoEl.title = "Motivo fixado — não pode ser alterado"
    }
    ...
    // Marca como fixado na memória para sobreviver a re-renders
    motivoRascunho[id] = motivo
    motivoRascunho[`_fixado_${id}`] = true
...
db.collection("solicitacoes")
  ...
  .onSnapshot(snapshot => {
    ...
    // Salva rascunhos antes de limpar a tabela
    document.querySelectorAll("#lista tr.pendente").forEach(tr => {
      const id = tr.querySelector(".btn-finalizar")?.getAttribute("onclick")?.match(/'([^']+)'/)?.[1]
      if (!id) return
      const c = document.getElementById(`c-${id}`)
      const m = document.getElementById(`m-${id}`)
      if (c) comentarioRascunho[id] = c.value
      if (m) motivoRascunho[id]     = m.value
    })
        ...
        lista.innerHTML += `
          <tr class="pendente">
            ...
            <td data-label="Motivo do Atraso">
              <input class="comment-input motivo-input" id="m-${doc.id}" placeholder="Motivo do atraso..." value="${d.motivoAtraso || ''}" ${d.motivoAtraso ? 'readonly style="background:#f1f5f9;color:#475569;border-color:#cbd5e1;cursor:not-allowed;" title="Motivo fixado — não pode ser alterado"' : ''}>
            </td>
            <td data-label="Ação">
              <button class="btn-fixar" id="btn-fixar-${doc.id}" onclick="fixarComentario('${doc.id}')" ${d.motivoAtraso ? 'disabled' : ''} style="${d.motivoAtraso ? 'opacity:0.5;cursor:default;' : ''}">${d.motivoAtraso ? '✓ Fixado' : '📌 Fixar Comentário'}</button>
              <button class="btn-finalizar" onclick="confirmar('${doc.id}')">✓ Finalizar</button>
            </td>
          </tr>
        `

        // Restaura rascunhos e conecta eventos para preservar durante re-renders
        requestAnimationFrame(() => {
          const cEl = document.getElementById(`c-${doc.id}`)
          const mEl = document.getElementById(`m-${doc.id}`)
          if (cEl) {
            if (comentarioRascunho[doc.id] !== undefined) cEl.value = comentarioRascunho[doc.id]
            cEl.addEventListener("input", () => { comentarioRascunho[doc.id] = cEl.value })
          }
          if (mEl) {
            if (motivoRascunho[doc.id] !== undefined) mEl.value = motivoRascunho[doc.id]
            mEl.addEventListener("input", () => { motivoRascunho[doc.id] = mEl.value })
            // Marca como fixado na memória se o Firebase já tem o motivo salvo
            if (d.motivoAtraso) motivoRascunho[`_fixado_${doc.id}`] = true
            // Se já foi fixado, re-aplica o bloqueio após re-render
            if (motivoRascunho[`_fixado_${doc.id}`]) {
              mEl.readOnly = true
              mEl.style.background = "#f1f5f9"
              mEl.style.color = "#475569"
              mEl.style.borderColor = "#cbd5e1"
              mEl.style.cursor = "not-allowed"
              mEl.title = "Motivo fixado — não pode ser alterado"
              const btn = document.getElementById(`btn-fixar-${doc.id}`)
              if (btn) { btn.textContent = "✓ Fixado"; btn.disabled = true }
            }
          }
        })
      }
