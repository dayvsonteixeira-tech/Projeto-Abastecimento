...
db.collection("solicitacoes")
  ...
  .onSnapshot(snapshot => {
    ...
    // Detecta mudanças para notificações
    snapshot.docChanges().forEach(change => {
      const d  = change.doc.data()
      const id = change.doc.id

      // Novo pedido → notifica abastecedor
      ...
      // Pedido finalizado → notifica o solicitante dono do pedido
  ...
  grafico = new Chart(ctx, {
    ...
    options: {
      ...
      plugins: {
        ...
        tooltip: {
          backgroundColor: "#1a1e29", borderColor: "#252a38", borderWidth: 1,
          titleColor: "#f0f2f7", bodyColor: "#8892a4", padding: 12,
          titleFont: { family: "'Space Mono', monospace", size: 11 },
          bodyFont:  { family: "'DM Sans', sans-serif", size: 13 }
        }
      },
      scales: {
        x: { grid: { color: "rgba(255,255,255,0.04)" }, ticks: { color: "#4e5668", font: { family: "'DM Sans', sans-serif", size: 11 } } },
        y: { beginAtZero: true, grid: { color: "rgba(255,255,255,0.04)" }, ticks: { color: "#4e5668", stepSize: 1, precision: 0, font: { family: "'Space Mono', monospace", size: 11 } } }
      }
    }
  })
}
...
// Pede permissão após 2s se ainda não decidiu
