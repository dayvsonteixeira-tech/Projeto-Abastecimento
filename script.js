...
function exportarExcel() {
  db.collection("solicitacoes")
    ...
    .get()
    .then(snapshot => {
    let csv = "Doca;Solicitante;Posição;Partnumber;Solicitado em;Abastecido em;Status\n";
  ...
  db.collection("solicitacoes").onSnapshot(snapshot => {
    ...
    snapshot.forEach(doc => {
      ...
      if (!d.abastecidoEm) {
        ...
        finalizadas++;
        const inicio = d.solicitadoEm.toDate();
        const fim = d.abastecidoEm.toDate();
    ...
    document.getElementById("kpiFinalizadas").innerText = finalizadas;

    if (qtdTempos > 0) {
      const media = somaTempos / qtdTempos;
      document.getElementById("kpiTempoMedio").innerText = formatarTempo(media);
    } else {
